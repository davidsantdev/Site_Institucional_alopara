...
const produtoSelecionado = ref<any>(null)
   ...
   const VinhoLimitados = ref(Vinho.slice(0, 5))
...
<div class="flex justify-center flex-col items-center" >
  ...
  <div class="flex justify-between w-[60%] items-center">
    ...
    <Button variant="link" @click="navigateTo('/Vinhos')" class="text-blue-500 text-[20px]">Ver mais</Button>
...
>
  ...
  <div class="flex items-center justify-center bg-slate-50 h-44 p-4 
    ...
    <img class="md:w-36 w-24 object-contain drop-shadow-md 
      group-hover:scale-105 transition-transform duration-300" :src="V.img" alt="">
  </div>
        ...
        <div class=" ">
         ...
         <div class="flex justify-center gap-3 mt-6">
          ...
          <div class=" flex gap-2 ">
            <button class="text-[#888] bg-[#1e1e1e] rounded-[10px] p-2 w-10 border-[1px] border-[#2a2a2a] " @click="diminuir(V.id)">-</button>
          <Button class="text-[#888] bg-[#1e1e1e] rounded-[10px] p-7 border-[1px] border-[#2a2a2a] ">{{ V.quantidade }}</Button>
          <button class="text-[#888] bg-[#1e1e1e] rounded-[10px] p-2 w-10 border-[1px] border-[#2a2a2a] " @click="aumentar(V.id)">+</button>
        </div>



           
           <Button @click="produtoSelecionado = V; adicionarCarrinho(V, V.quantidade)" class="bg-[#cc1e1e] text-white p-7  font-bold text-[16px] " > Adicionar ao carrinho <span> <ShoppingBasket/> </span> </Button> 
