...
const { produtos: vinhoLimitados, carregando } = useVitrine('/api/bebidas', { busca: 'vinho', limite: 5 })

const produtoSelecionado = ref<any>(null)

function aumentar(id: string) {
  const item = vinhoLimitados.value.find(p => p.id === id)
  if (item)
    item.quantidade++
}
...
<template>
  <div>
    <section v-if="carregando || vinhoLimitados.length" class="w-full max-w-6xl mx-auto px-4 md:px-6 mb-14">
      <!-- cabeçalho -->
      <div class="flex items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-3">
          <span class="text-red-600 text-[10px] font-black tracking-[4px] uppercase">Vinhos</span>
      ...
      <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4">
        <Dialog v-for="V in vinhoLimitados" :key="V.id">
          <DialogTrigger as-child>
            <div class="group relative flex flex-col bg-(--bg-cartao) border border-(--borda) rounded-2xl overflow-hidden cursor-pointer hover:border-red-600 hover:-translate-y-1 transition-all duration-300">
              ...
              <div v-if="V.emPromocao" class="absolute top-2.5 left-2.5 z-10 bg-red-600 text-white text-[9px] font-black tracking-wide px-2 py-1 rounded-full uppercase">
                -{{ percentualDesconto(V.precoOriginal, V.preco2) }}%
              </div>
                ...
                <div class="mt-auto">
                  <template v-if="V.emPromocao">
                    ...
                    <p class="leading-tight">
                      ...
                      <span class="text-lg md:text-xl font-extrabold text-(--preco)">R$ {{ V.preco2 }}</span>
                    </p>
                  </template>
          ...
          <DialogContent class="bg-(--bg-cartao) border border-(--borda) rounded-3xl p-0 md:w-120 w-[92vw] overflow-hidden">
            ...
            <div class="px-7 py-6">
              <span class="text-[11px] text-(--texto-suave) uppercase tracking-widest font-bold">{{ V.tipo }}</span>
              <h2 class="mt-1 text-xl font-bold text-(--texto-primario) leading-snug">
                {{ V.nome }}
              </h2>
              <div class="mt-2">
                <p v-if="V.emPromocao" class="text-sm text-(--texto-fraco)">
                  Preço normal: <span class="line-through">R$ {{ V.precoOriginal }}</span>
                </p>
              ...
              <div class="mt-6 flex items-center gap-3">
                <div class="flex items-center gap-1 rounded-xl bg-(--bg-elevado) border border-(--borda-forte) p-1">
                  ...
                  <span class="min-w-8 text-center font-bold text-(--texto-primario)">{{ V.quantidade }}</span>
                  <button
                    type="button"
