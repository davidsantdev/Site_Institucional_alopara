...
const { produtos: perfumariaLimitados, carregando } = useVitrine('/api/higiene', { limite: 5 })

const produtoSelecionado = ref<any>(null)

function aumentar(id: string) {
  const item = perfumariaLimitados.value.find(p => p.id === id)
  if (item)
    item.quantidade++
}
...
<template>
  <section v-if="carregando || perfumariaLimitados.length" class="w-full max-w-6xl mx-auto px-4 md:px-6 mb-14">
    <!-- cabeçalho -->
    <div class="flex items-center justify-between gap-4 mb-6">
      <div class="flex items-center gap-3">
        <span class="text-red-600 text-[10px] font-black tracking-[4px] uppercase">Perfumaria</span>
    ...
    <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4">
      <Dialog v-for="L in perfumariaLimitados" :key="L.id">
        <DialogTrigger as-child>
          <div class="group relative flex flex-col bg-(--bg-cartao) border border-(--borda) rounded-2xl overflow-hidden cursor-pointer hover:border-red-600 hover:-translate-y-1 transition-all duration-300">
            ...
            <div v-if="L.emPromocao" class="absolute top-2.5 left-2.5 z-10 bg-red-600 text-white text-[9px] font-black tracking-wide px-2 py-1 rounded-full uppercase">
              -{{ percentualDesconto(L.precoOriginal, L.preco2) }}%
            </div>
            ...
            <div class="flex flex-1 flex-col gap-2 p-3">
              ...
              <div class="mt-auto">
                <template v-if="L.emPromocao">
                  <p class="text-[9px] leading-tight text-(--texto-fraco)">
                    Preço normal: <span class="line-through">R$ {{ L.precoOriginal }}</span>
                  </p>
        ...
        <DialogContent class="bg-(--bg-cartao) border border-(--borda) rounded-3xl p-0 md:w-120 w-[92vw] overflow-hidden">
          ...
          <div class="px-7 py-6">
            <span class="text-[11px] text-(--texto-suave) uppercase tracking-widest font-bold">{{ L.tipo }}</span>
            <h2 class="mt-1 text-xl font-bold text-(--texto-primario) leading-snug">
              {{ L.nome }}
            </h2>
            <div class="mt-2">
              <p v-if="L.emPromocao" class="text-sm text-(--texto-fraco)">
                Preço normal: <span class="line-through">R$ {{ L.precoOriginal }}</span>
              </p>
            ...
            <div class="mt-6 flex items-center gap-3">
              <div class="flex items-center gap-1 rounded-xl bg-(--bg-elevado) border border-(--borda-forte) p-1">
                ...
                <span class="min-w-8 text-center font-bold text-(--texto-primario)">{{ L.quantidade }}</span>
                <button
                  type="button"
