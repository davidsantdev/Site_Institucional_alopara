...
function comprarWhatsapp() {
  ...
  const mensagem = carrinho.value
    .map(
      (item) => `Olá, vim pelo site
Produto: ${item.nome}
          ...
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sticky top-6">
            ...
            <div class="flex flex-col gap-2 mb-4">
              <div v-for="C in carrinho" :key="C.id"
                ...
                <span class="truncate max-w-[60%]">{{ C.nome }} x{{ C.quantidade ?? 1 }}</span>
                <span class="font-medium text-slate-700">R$ {{ precoItem(C) }}</span>
              </div>
            </div>
