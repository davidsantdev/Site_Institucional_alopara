...
async function buscarProdutos(pagina = 1) {
  ...
  try {
    ...
    cacheCompleto.value = res.cacheCompleto ?? false
...
function iniciarPolling() {
  ...
  pollingTimer = setInterval(async () => {
    try {
      ...
      const novoTotal = res.total || 0

      if (novoTotal > totalProdutos.value || res.produtos?.length > produtos.value.length) {
        produtos.value      = res.produtos      || []
        totalProdutos.value = novoTotal
        totalPaginas.value  = res.totalPaginas  || 1
      }
  ...
  <div class="min-h-screen bg-[#F2F3F5] flex flex-col font-sans">
      ...
      <div class="relative mx-auto max-w-5xl text-center">
        ...
        <p class="mt-2 text-white/80 text-base md:text-lg flex items-center justify-center gap-2">
          <template v-if="totalProdutos > 0">
            ...
            <span v-if="!cacheCompleto" class="inline-flex items-center gap-1 text-sm text-white/70">
    ...
    <div v-if="erro" class="flex flex-1 items-center justify-center py-32">
      <div class="text-center">
        ...
        <button @click="buscarProdutos(1)" class="mt-6 rounded-xl bg-pink-500 px-6 py-3 text-white font-bold hover:bg-pink-600 transition">
          Tentar novamente
        </button>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <!-- ===== SKELETON (só no primeiro carregamento) ===== -->
      <div v-if="carregando && totalProdutos === 0" class="mx-auto w-full max-w-7xl px-3 py-6 md:px-6">
      ...
      <div v-else class="mx-auto w-full max-w-7xl px-3 py-6 md:px-6">
        ...
        <div class="mb-4 flex items-center justify-between">
          <p class="text-sm text-gray-500 flex items-center gap-2">
            ...
            <span
              v-if="!cacheCompleto"
              class="inline-flex items-center gap-1 rounded-full bg-pink-100 px-2 py-0.5 text-[11px] text-pink-600 font-medium"
            >
              <RefreshCw :size="10" class="animate-spin" />
              atualizando
            </span>
          </p>
          <p class="text-sm text-gray-400">Página {{ paginaAtual }} de {{ totalPaginas }}</p>
        </div>
        ...
        <div v-if="totalPaginas > 1" class="mt-10 flex flex-col items-center gap-3">
          <div v-if="!cacheCompleto" class="flex items-center gap-2 text-xs text-pink-500 mb-1">
            <RefreshCw :size="11" class="animate-spin" />
            Catálogo carregando — mais páginas aparecerão em breve
          </div>
          ...
          <div class="flex items-center gap-1">
            <button
              ...
              class="flex h-9 w-9 items-center justify-center rounded-xl border border-gray-200 bg-white text-gray-500 hover:bg-pink-50 hover:text-pink-500 hover:border-pink-200 disabled:opacity-30 disabled:cursor-not-allowed transition"
            ><ChevronLeft :size="16" /></button>

            <template v-for="(p, idx) in paginasVisiveis" :key="idx">
              <span v-if="p === '...'" class="flex h-9 w-9 items-center justify-center text-gray-400 text-sm select-none">…</span>
              <button
                v-else
                ...
                :class="[
                  ...
                  p === paginaAtual
                    ...
                    : 'border border-gray-200 bg-white text-gray-600 hover:bg-pink-50 hover:text-pink-500 hover:border-pink-200'
                ]"
              >{{ p }}</button>
            </template>
      ...
      >
        ...
        <div class="relative z-10 w-full max-w-md overflow-hidden rounded-t-3xl md:rounded-3xl bg-[#111] shadow-2xl">
          <div class="flex items-center justify-center bg-white h-60 p-6 relative">
            <button @click="fecharModal" class="absolute top-4 right-4 rounded-full bg-gray-100 p-2 text-gray-500 hover:bg-gray-200 transition">
              <X :size="18" />
            </button>
            <img :src="imgSrc(modalProduto.img)" :alt="modalProduto.nome" @error="imagemErro" class="h-48 object-contain drop-shadow-lg" />
          </div>
          <div class="px-8 py-6">
