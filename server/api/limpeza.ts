...
const CACHE_FILE = join(process.cwd(), '.cache', 'limpeza.json')

// Departamentos relacionados a limpeza / higiene / casa
const DEPS_LIMPEZA = [
  'LIMPEZA',
...
async function lerCacheDisco(): Promise<CacheProdutos | null> {
  try {
    ...
    console.log(`💾 [Limpeza] Cache do disco: ${data.data.length} produtos (${new Date(data.timestamp).toLocaleTimeString('pt-BR')})`)
    return data
  } catch {
    return null
  }
...
async function salvarCacheDisco(cache: CacheProdutos): Promise<void> {
  try {
    ...
    console.log(`💾 [Limpeza] Cache salvo: ${cache.data.length} produtos`)
  } catch (err) {
    console.error('[Limpeza] Erro ao salvar cache em disco:', err)
  }
}
...
function criarSemaforo(max: number) {
  ...
  function reduzir() {
    if (limite > CONCORRENCIA_MIN) {
      ...
      console.warn(`⚠️  [Limpeza] Semáforo reduzido para ${limite} (muitos timeouts)`)
    }
  }
  ...
  for (let tentativa = 1; tentativa <= MAX_RETRIES; tentativa++) {
    ...
    try {
      ...
      if (tentativa < MAX_RETRIES) {
        ...
        console.error(`[Limpeza] ❌ Pág ${pagina} desistida:`, ehTimeout ? 'timeout' : err?.message)
        return []
      }
    } finally {
      semaforo.liberar()
...
function normalizarProdutos(brutos: any[], ids: Set<string>): Produto[] {
  ...
  for (const p of brutos) {
    ...
    if (!DEPS_LIMPEZA.some((d) => dep.includes(d))) continue
...
async function buscarTodosProdutos(token: string): Promise<Produto[]> {
  ...
  if (disco && disco.data.length > 0) {
    ...
    if (precisaAtualizar && !backgroundRodando) {
      console.log('[Limpeza] 🔄 Cache do disco antigo — atualizando em background...')
  ...
  // ── 4. Sem cache nenhum: lote inicial paralelo ────────────────────────────
  console.log(`[Limpeza] 🚀 Lote inicial (${PAGINAS_INICIAIS} páginas paralelas)...`)
  ...
  console.log('[Limpeza] ✅ Lote inicial:', produtosIniciais.length, 'produtos')
    ...
    try {
      ...
      while (pagina <= MAX_PAGINAS_API) {
        ...
        if (novosNoLote > 0) {
          ...
          console.log(`[Limpeza] 📦 Págs ${pagina}–${pagina + TAMANHO_LOTE_BG - 1}: +${novosNoLote} → ${todos.length} total`)
        }

        if (paginasVazias >= MAX_PAGINAS_VAZIAS) {
          console.log(`[Limpeza] 🏁 API esgotada`)
          break
        }
      ...
      console.log('[Limpeza] 🏁 Cache completo:', todos.length, 'produtos')
...
function agendarRenovacao(headers: Record<string, string>) {
  setTimeout(async () => {
    console.log('[Limpeza] 🔄 Renovando cache proativamente...')
    try {
      if (produtosCache) produtosCache = { ...produtosCache, completo: false, timestamp: 0 }
      rodarBackground(headers, 1)
    } catch (err) {
      console.error('[Limpeza] Erro na renovação proativa:', err)
...
async function warmupCache() {
  ...
  warmupFeito = true
  console.log('[Limpeza] 🔥 Boot...')

  const disco = await lerCacheDisco()
  if (disco && disco.data.length > 0) {
    produtosCache = disco
    console.log(`[Limpeza] ⚡ Memória pré-aquecida com ${disco.data.length} produtos do disco`)

    if (Date.now() - disco.timestamp < RENOVAR_EM) {
      const tempoRestante = RENOVAR_EM - (Date.now() - disco.timestamp)
      console.log(`[Limpeza] ⏱ Próxima renovação em ${Math.round(tempoRestante / 60000)}min`)
      try {
  ...
  try {
    ...
    console.error('[Limpeza] Erro no warmup:', err)
    warmupFeito = false
  }
}
...
export default defineEventHandler(async (event) => {
  try {
    ...
    console.error('[Limpeza] ERRO API:', err)
    return {
